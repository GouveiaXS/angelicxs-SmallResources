...
RegisterNetEvent('angelicxs-CarryMeSennnpai!', function()
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				if IsEntityPlayingAnim(playerPed, 'missminuteman_1ig_2', 'handsup_base', 3) or IsEntityPlayingAnim(playerPed, 'mp_arresting', 'idle', 3) or IsEntityPlayingAnim(playerPed, 'dead', 'dead_a', 3) or IsEntityPlayingAnim(playerPed, 'combat@damage@writhe', 'writhe_loop', 3)	then
					...
					QBCore.Functions.Notify("You can\'t carry a person like this!", "error")
				end
			else
				QBCore.Functions.Notify("No one is close enough to carry!", "error")
			end
		else
			QBCore.Functions.Notify("No one is close enough to carry!", "error")
		end
